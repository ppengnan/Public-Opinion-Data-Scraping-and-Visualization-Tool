...
def process_time(publish_time):  
    ...
    # 检查是否包含"年"，如果没有，则补充当前年份
    if "年" not in publish_time:
        current_year = time.strftime("%Y", time.localtime(now))  # 获取当前年份
        publish_time = current_year + "年" + publish_time  # 将当前年份加到发布时间前 
